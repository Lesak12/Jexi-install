...
preflight(){
    ...
    output "Copyright © 2024 Vikas Dongre <zvikasdongre@gmail.com>."
    output ""

    output "Harap dicatat bahwa skrip ini dimaksudkan untuk diinstal pada OS baru. Menginstalnya pada OS yang tidak baru dapat menyebabkan masalah."
    output "Deteksi sistem operasi otomatis diinisialisasi..."
...
os_check(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if  [ "$dist_version" != "22.04" ]; then
            output "Versi Ubuntu tidak didukung. Hanya Ubuntu 22.04 yang didukung."
            exit 2
        fi
    elif [ "$lsb_dist" = "debian" ]; then
        ...
        output "Sistem operasi tidak didukung."
        output ""
        output "Supported OS:"
        output "Ubuntu: 22.04"
        output "Debian: 11"
...
install_options(){
    output "Silakan pilih opsi instalasi Anda:"
    output "[1] Pasang panel ${PANEL}."
    output "[2] Pasang wings ${WINGS}."
    output "[3] Pasang panel ${PANEL} dan wings ${WINGS}."
    output "[4] Upgrade panel ke ${PANEL}."
    output "[5] Upgrade wings ke ${WINGS}."
    output "[6] Upgrade panel ke ${PANEL} dan daemon ke ${WINGS}."
    output "[7] Pasang phpMyAdmin (hanya gunakan ini setelah Anda menginstal panel)."
    output "[8] Emergency MariaDB root password reset."
    output "[9] Emergency database host information reset."
    read -r choice
    case $choice in
        1 ) installoption=1
            output "Anda telah memilih pemasangan panel ${PANEL} saja."
            ;;
        2 ) installoption=2
            output "Anda telah memilih instalasi sayap ${WINGS} saja."
            ;;
        3 ) installoption=3
            output "Anda telah memilih pemasangan ${PANEL} panel dan sayap ${WINGS}."
            ;;
        4 ) installoption=4
            output "Anda telah memilih untuk meningkatkan panel ke ${PANEL}."
            ;;
	      5 ) installoption=5
            output "Anda telah memilih untuk mengupgrade daemon ke ${DAEMON}."
            ;;
        6 ) installoption=6
            output "Anda telah memilih untuk mengupgrade panel ke ${PANEL} dan daemon ke ${DAEMON}."
            ;;
        7 ) installoption=7
            output "Anda telah memilih untuk menginstal phpMyAdmin."
            ;;
        8 ) installoption=8
            output "Anda telah memilih pengaturan ulang kata sandi root MariaDB."
            ;;
        9 ) installoption=9
            output "Anda telah memilih pengaturan ulang informasi Host Basis Data."
            ;;
        * ) output "Anda tidak memasukkan pilihan yang valid."
            install_options
    esac
}
